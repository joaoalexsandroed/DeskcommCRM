...
describe("createDefaultRegistry", () => {
  ...
  it("cada factory produz um LanguageModel (não lança ao instanciar)", () => {
    ...
    // Nota de integração: a main oficial também testa um `baseUrl` de gateway
    // próprio no 3º parâmetro — não trazido nesta integração (ver providers.ts).
    expect(() => reg.openrouter!("k", "anthropic/claude-sonnet-4-6")).not.toThrow();
  });
});
