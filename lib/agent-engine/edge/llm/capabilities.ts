...
export function modelCapabilities(provider: string, modelId: string): ModelCapabilities {
  ...
  // (conservador), não um chute. Vendor desconhecido (meta-llama, deepseek,
  // x-ai etc.) cai no NONE conservador — mídia ainda "funciona" via derivado
  // textual, só não vai nativa.
  if (ROTEADORES.has(p ?? "")) {
